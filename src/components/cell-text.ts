...
// El memo de `cellTextFor`, explicado en su doc: 96 entradas como maximo, y una por
// orientacion en vez de una por render. Modulo y no componente a proposito.
 ...
 * reflexiones son 96 entradas—, asi que el memo vive en el modulo y no adentro del
 * componente: asi sobrevive al render en vez de rearmarse en cada uno. No es estado de
 * la app —no lo puede observar nadie, y para la misma entrada devuelve siempre lo
 * mismo—, es el mismo argumento con el que `palette.constants.ts` guarda `fg` en vez
 * de recalcular la luminancia.
 *
 ...
 */
export function cellTextFor(piece: PieceKey, rotation: number, mirror: boolean): readonly CellText[] {
  const key = `${piece}${rotation}${mirror}`;
  const hit = memo.get(key);
  if (hit) return hit;
  const arp = notesForRotation(BASE_MAP[piece], DEFAULT_OCTAVE, rotation);
