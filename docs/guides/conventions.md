...
**La verifica el linter, no la revisión.** Desde el spec 029 la verifica por **ruta**:
...
**Y dejó de ser una red.** Hasta el 029 se prohibía el *string* del import, así que los patrones tenían
...
`invariants.ts` como hojas que no se importan entre sí—. Desde el 029 esas cinco filas se expanden a
...
**Lo verifica el linter en `domain/` y en `audio/`, y no en `components/`** (spec 029). La línea es la
