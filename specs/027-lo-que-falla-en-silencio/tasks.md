...
- [ ] T001 `components/__tests__/route-source.test.ts`: el test de `research.md` §1, **en rojo primero**.
      ...
      dar `[]` y hoy da 5 — **AC1**
- [ ] T002 `src/components/route-source.ts`: `reiniciar()`, que devuelve `activa`, `pendiente`,
...
- [ ] T004 `src/App.tsx::resetBoard()`: llamarla junto a `frenarTransporte()`, **por `use-engine.ts`**
      — que la re-exporta como ya re-exporta `stopClock()`—, sin que el shell importe
      `components/route-source.ts` — **AC12**
- [ ] T005 `src/components/__tests__/route-source.test.ts` — **el otro test, el que evita pasarse de
...
- [ ] T008 `src/audio/engine.ts`: en el `catch`, `ctx = null`, `master = null`, `analyser = null`. El
...
- [ ] T010 Comentario con el modo de falla completo, que es lo que hace legible una línea que si no
      ...
      pura no puede ver
- [ ] T011 `src/audio/__tests__/engine.browser.test.tsx`: forzando un fallo posterior al constructor,
...
- [ ] T012 `src/audio/__tests__/engine.browser.test.tsx`: un fallo total produce **un** warning y no uno
      por llamada — **AC4**

## Paso 3 — El reposo de `Spectrum`

- [ ] T013 `src/components/spectrum-loop.ts` —no el `.tsx`, el 029 mudó el loop—: **clave de lo último
      dibujado** y no un booleano, la misma forma que `dibujado` en `playhead-loop.ts`; `drawIdle` sólo
      en la transición — **AC5**
- [ ] T014 El `resize` la invalida: redimensionar borra el canvas, así que hay que volver a pintarlo
...
- [ ] T016 `src/components/__tests__/Spectrum.browser.test.tsx`: en reposo no vuelve a dibujar después
...
- [ ] T017 `src/__tests__/App.browser.test.tsx`: contar ejecuciones de `OrientationPanel` mientras el
...
- [ ] T018 Anotar el número en `src/App.tsx:253-257`. Hoy tiene que dar **diez además del render
...
- [ ] T022 [P] `src/audio/engine.ts:129`: `const bus = master;` después de la guarda, y el `!` se va. TS
      pierde el estrechamiento al entrar al closure del `forEach` porque `master` es un `let` de módulo
      — **AC8**
- [ ] T023 [P] `src/main.tsx:12`: comentario de una línea con por qué ese `!` **se queda** — es el idiom de la
      ...
      **AC8**
- [ ] T024 `CLAUDE.md:170-174`: la regla «cero `any` y cero `@ts-ignore`» pasa a nombrar también la
...
- [ ] T025 Confirmar **AC10**: cero cambio visual y cero cambio de audio. Los 562 de `src/` y los del
...
- [ ] T026 `pnpm verify` verde con sus cuatro nodos, incluida la pasada de `coverage` con umbral **100**
...
- [ ] T033 `src/components/use-engine.ts`: re-exportar el reinicio de la cola de dibujo al lado de
...
- [ ] T034 `src/components/__tests__/use-engine.browser.test.tsx`: su `vi.mock('../route-source.ts')`
...
- [ ] T035 `src/audio/__tests__/engine.browser.test.tsx:503`: **reescribir** «con el grafo a medio
...
- [ ] T036 `src/audio/engine.ts`: comentario en las guardas `if (!c || !master)` de `playNotes` y
      ...
      `/* v8 ignore */` — **AC11**
- [ ] T037 `src/components/__tests__/Spectrum.browser.test.tsx`: la tercera transición —**señal →
...
- [ ] T038 `docs/architecture/audio.md`: la sección de `route-source.ts` (línea ~504) describe el par
...
- [ ] T039 `specs/revisiones.md`: anotar lo aprendido, que es de la familia del pase de mutación del 029
...
- [ ] T041 `src/components/Playhead.tsx:35-38`: el docblock afirma en presente una carencia que **ya
