...
- [ ] T006 Ídem «Recorrido en el vacío», etiquetado por el `<span>` de `:116` — **AC4**
- [ ] T007 Los cuatro botones de rotación: `id` en el `<span>Rotación</span>` (`PiecePalette.tsx:73`),
...
- [ ] T008 Ídem los dos de régimen: `id` en el `<span>cambia</span>` (`:81`) y `role="group"` en el
...
- [ ] T010 `type="button"` en los 22 `<button>`, que salen de **siete** sitios de JSX y no de
...
- [ ] T015 `DESIGN.md:120`, «El color comunica identidad, nunca estado», gana su mitad no visual: si el
...
- [ ] T016 [P] `TransportPanel.browser.test.tsx` (ya existe): `page.getByRole('slider', { name: 'Tempo' })`
      existe y su `aria-valuetext` es `110 bpm` — **AC8**
- [ ] T017 [P] `PiecePalette.browser.test.tsx` (ya existe): `page.getByRole('button', { name: /^Reflexión$/, pressed: false })`,
...
- [ ] T018 Test: los seis botones de los dos grupos exponen `aria-pressed`, y exactamente **uno** de
      los cuatro de rotación está en `true`. **Sin `[P]`: mismo archivo que T017** — **AC8**
- [ ] T019 Los tests consultan por **rol y nombre**, nunca por `className`, y con `page` de
...
- [ ] T028 [P] `OrientationPanel.tsx:80-84`: `aria-pressed={activo}` en las doce miniaturas. Es el
...
- [ ] T029 `OrientationPanel.browser.test.tsx` (ya existe): las doce exponen `aria-pressed` y
      exactamente una está en `true` — **AC8**, **AC11**
- [ ] T030 Test: los 22 botones renderizados declaran `type="button"`. Sin esto AC6 es el único cambio
