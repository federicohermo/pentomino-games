...
- [x] T001 `pathThroughCells(cells, tiebreak)` en `src/domain/transform.ts`: matriz de costos con
...
- [x] T002 Docblock de `pathThroughCells`: qué devuelve (orden de visita **por posición**, no grado por
      celda), por qué `tiebreak` entra por parámetro (D5), por qué el costo se empaqueta en un entero
      (D6) y cuál es el dominio de `n`
- [x] T003 [P] Test de referencia en `src/domain/__tests__/transform.test.ts`: fuerza bruta sobre las
      permutaciones **escrita en el test**, contrastada sobre las 12 piezas y 400 formas aleatorias con
      semilla fija (AC2)
- [x] T004 [P] Test: el resultado es una permutación de `0..n-1`, y casos borde de 0, 1 y 2 celdas
- [x] T005 [P] Test: las 12 piezas tienen distancias no crecientes (AC3)
- [x] T006 [P] Test: `I L N P U V W Z` dan 0 saltos y `F T Y X` dan 1, 1, 1 y 2 (AC4)
- [x] T007 [P] Test: el desempate es determinista y se ejerce donde hay empate real — `Y` y `X` (AC6)

## Paso 2 — Cablearlo (**cambia lo que suena**, commit propio)

- [x] T010 `angularRank` interna en `src/domain/music.ts`: el cuerpo de `degreeByCellIndex` de hoy, sin
      exportar
- [x] T011 `degreeByCellIndex` pasa a componer `pathThroughCells(cells, angularRank(cells))` y a
      invertir el orden a grados. Firma y contrato intactos (AC7)
- [x] T012 Docblock de `degreeByCellIndex` reescrito: el camino como regla, el ángulo como desempate de
...
- [x] T013 Test testigo de la `U` en `src/domain/__tests__/music.test.ts`: recorre sus cinco celdas sin
      saltar, con la colocación del spec (AC1)
- [x] T014 Migrar `AC2 — en I y X la celda parada sobre el centroide se lleva el grado 0`: lo supera D3.
      Se reemplaza afirmando lo que ahora es cierto —que `I` se camina entera— con el motivo escrito
- [x] T015 Migrar los dos tests de `AC4 — el desempate a igual angulo`: el desempate angular sigue
      existiendo pero elige la **dirección** del camino, no los grados
- [x] T016 Recongelar `AC5 — la referencia congelada`: las 60 notas de `research.md` §5 escritas a mano,
      `TONICA_EN` nuevo, y el docblock diciendo que la fuente es este spec y no la lámina del 007 (AC8, D7)
- [x] T017 Remedir el test de `recalcular sobre la forma ya transformada`: el 75 de 96 es un número del
      anillo angular. Lo que hay que verificar es el arrastre por índice (AC5)
- [x] T018 Test: el camino sigue siendo camino en las 8 orientaciones de las 12 piezas — 0 de 96 rompen
      (AC5, D4)

## Paso 3 — Los valores aguas abajo

- [x] T020 `src/domain/__tests__/sequence.test.ts`: los grados y las puertas escritos a mano. Cada uno se
      mira antes de tocarlo; lo que cambia es el valor esperado, no la intención (AC9)
- [x] T021 [P] `mcp-server/src/__tests__/render.test.ts:71,86`: los grados de la `X` al renderizar
- [x] T022 [P] Si algún test afirma algo que este spec vuelve falso, se declara superado con su motivo
      escrito en el propio test — no se borra en silencio (AC9)
- [x] T023 **Los tres testigos del cruce del 011** (`sequence.test.ts`, `route-source.test.ts`,
      `mcp-server/src/__tests__/tools.test.ts`): el tablero de la `X` deja de cruzar porque su centro deja
      de ser puerta (D9). Se mudan los tres al mismo tablero nuevo, con guarda de cruces exactos (AC14)
- [x] T024 **Los tres tableros que ejercen un empate** del circuito se vuelven a buscar por fuerza bruta:
      el empate depende de las puertas, y heredarlo deja el test verde sin ejercer nada (AC14)

## Paso 4 — Documentación

- [x] T030 [P] `docs/architecture/modelo-musical.md`: la fila «La forma» de la tabla de derivaciones y la
      sección «forma → qué celda tiene qué nota» entera, más la nota de Reproducción que llama al orden
      «el orden angular» (AC13)
- [x] T031 [P] `CLAUDE.md`: la fila del modelo musical en la tabla de documentación (AC13)
- [x] T032 [P] `.claude/rules/domain.md`: la mención al anillo angular (AC13)

## Paso 5 — Verificación

- [x] T040 `pnpm verify` en verde (AC11)
- [x] T041 `check_invariants` en **proceso fresco**, antes y después (AC11)
- [x] T042 [P] `describe_piece` sobre `U`, `I`, `Y` y `X`
- [x] T043 [P] `simulate_board` sobre el tablero de las capturas del pedido: las puertas nuevas
