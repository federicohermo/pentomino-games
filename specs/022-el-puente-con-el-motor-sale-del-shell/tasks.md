...
- [ ] T001 `src/components/types/motor.types.ts`: `MotorDeTransporte` (`arrancar`, `frenar`,
...
- [ ] T002 `src/components/motor.ts`: `proyectarAlMotor(s: Sequence): SequenceDelMotor`. Los dos
...
- [ ] T003 `motor.ts`: el comentario del ternario del click viaja **desde `App.tsx`**, no se reescribe.
...
- [ ] T004 `motor.ts`: comentario sobre qué **cae** en la proyección y por qué —`pieceId` porque el
...
- [ ] T005 `motor.ts`: `alternarTransporte(playing, motor)`. El `return motor.corriendo()` y **no**
...
- [ ] T006 `src/components/__tests__/motor.test.ts`: los tres casos de `proyectarAlMotor`, armados con
      ...
      **AC2**, **AC3**
- [ ] T007 `motor.test.ts`: las dos ramas de `alternarTransporte` con motores falsos. La segunda
...
- [ ] T008 `src/components/use-motor.ts`: `useMotorSincronizado({ secuencia, placed, tempo, clicks })`
...
- [ ] T009 `use-motor.ts`: las dos proyecciones llaman a `proyectarAlMotor`. Es lo único que cambia
      adentro de los efectos — **AC1**
- [ ] T010 `use-motor.ts`: docblock del módulo con **por qué `secuencia` entra por parámetro y el hook
...
- [ ] T011 `use-motor.ts`: `MOTOR` como const de módulo, cableando las tres funciones reales al rol de
...
- [ ] T012 `src/App.tsx`: borrar los cuatro efectos (`:98-99` y `:182-258`) y llamar al hook **después
...
- [ ] T013 `App.tsx`: `togglePlay` pasa a `setPlaying(alternarTransporte(playing, MOTOR))` y
...
- [ ] T014 `App.tsx`: actualizar el docblock del módulo (`:28-44`), que hoy describe el archivo como
      «estado, derivados, handlers y efectos»
- [ ] T015 `pnpm verify` en verde
...
- [ ] T042 `src/components/use-entrada.ts`: `useAtajosDeTeclado(acciones, tapLimpio)` y
      ...
      — **AC14**
- [ ] T043 `use-entrada.ts`: **`tapLimpio` entra por parámetro a los DOS hooks y NO se muda adentro de
...
- [ ] T044 `use-entrada.ts`: el handler de la rueda conserva `{ passive: false }` explícito **y el
...
- [ ] T045 `src/App.tsx`: `alRotar` va envuelto en `useCallback(…, [])`. Sin eso el listener de
      ...
      **AC16**
- [ ] T046 `App.tsx`: los tres callbacks del teclado **no** se memoizan a `[]`. Dependen de `rotation`
...
- [ ] T063 `use-entrada.ts`: **`useAtajosDeTeclado` no puede poner el objeto `acciones` en el array de
      ...
      cubre — **AC16**
- [ ] T047 `App.tsx`: borrar los dos efectos (`:286-372`) y llamar a los dos hooks. `handleContextMenu`
      **no se muda**: no pasa por ningún efecto — **AC14**
- [ ] T048 Oráculo de **AC14**: `grep -c "useEffect" src/App.tsx` devuelve **0**, y el import de React
      pierde `useEffect`

## Paso 4 — La paleta en tres archivos

- [ ] T049 `src/components/types/panel.types.ts`: los dos objetos de props. `regimen` y `noteSet` van
...
- [ ] T050 `src/components/PanelDeOrientacion.tsx`: las doce miniaturas, el régimen, la reflexión y el
...
- [ ] T051 `src/components/PanelDeTransporte.tsx`: **sólo el bloque `border-t` de `:256-299`** —Tempo,
...
- [ ] T052 `PiecePalette.tsx`: se queda con la tarjeta, la composición y —medido, no por gusto— **la
...
- [ ] T064 `PiecePalette.tsx` / `PanelDeOrientacion.tsx`: los comentarios de medición **viajan con el
...
- [ ] T053 `App.tsx`: los dos objetos se arman **inline en el JSX**. Comentario al lado: identidad
...
- [ ] T016 `pnpm remove` de las cuatro `@testing-library/*`, `@types/jest`, `postcss` y `autoprefixer`,
      **en su propio commit**: es un borrado, y la regla del repo es que revertirlo sea trivial —
      **AC7**
- [ ] T017 `vite.config.ts`: se va el comentario que explica cómo esquivar a `@types/jest` (`:14-16`;
...
- [ ] T018 `specs/021-el-tablero-es-la-pantalla/tasks.md`: `T059`, `T060` y `T049` actualizan «los seis
...
- [ ] T039 `specs/018-la-pieza-se-elige-con-su-letra/tasks.md`: su `T014` cita **`App.tsx:311`** (el
...
- [ ] T040 `specs/019-el-panel-se-queda-sin-botones/tasks.md`: su `T040` afirma que «`resetBoard`
...
- [ ] T041 `specs/020-la-orientacion-es-de-la-pieza/tasks.md`: su `T019` abre con «`resetBoard` **no
...
- [ ] T061 `src/`: los **cinco docblocks** que este spec vuelve falsos, medidos en `research.md` §14.
...
- [ ] T019 [P] `CLAUDE.md:74` — **AC9**
- [ ] T020 [P] `docs/architecture/overview.md`: **`:23`** (el diagrama ASCII), **`:68`** (la prosa) y
...
- [ ] T021 [P] `.claude/rules/ui.md:9-11`: dos cosas en la misma frase. «Los seis efectos» (**AC9**) y
...
- [ ] T022 [P] `docs/guides/conventions.md`: la regla de idioma de los identificadores. **Descriptiva**:
...
- [ ] T023 `docs/guides/conventions.md`: el criterio de comentario en el eje del **tiempo** — se
...
- [ ] T024 `specs/deuda.md`: **borrar** los tres ítems que este spec cierra —las `@testing-library/*`
      ...
      a ojo — **AC8**
- [ ] T025 `specs/deuda.md`: **no se agrega ningún ítem nuevo.** Con el spec ampliado a los seis
...
- [ ] T054 `specs/018-la-pieza-se-elige-con-su-letra/tasks.md`: `T013` y `T015` escriben «el efecto de
...
- [ ] T055 `specs/020-la-orientacion-es-de-la-pieza/tasks.md`: `T009`, `T010`, `T011` y `T040` apuntan
      ...
      **AC22**
- [ ] T056 `specs/019-…/tasks.md` (`T001`, `T002`, `T003`, `T005`, `T007`, `T011`, `T020`, **`T031`**,
...
- [ ] T057 Los **tres casos testigo** primero, y solos: `App.tsx:53-66` (el default de `clicks` que
      ...
      nada más — **AC19**
- [ ] T058 El pase sobre el resto de `src/` sin tests, con las cinco reglas del paso 8 del plan.
      **Nada se borra: se mueve** a `revisiones.md`, con fecha y con el spec que lo originó. Ante la
      duda, se queda — **AC19**
- [ ] T059 El commit del pase va **solo**, y es lo último del spec: revertirlo entero tiene que ser
      trivial, y sobre su diff se verifica la conservación — **AC20**
- [ ] T060 Oráculo de **AC19**: lo que `src/` perdió en comentarios está en `revisiones.md`. Se lee
...
- [ ] T029 Oráculo de **AC1**: `grep -rn "{ offset: c.offset, note: c.note }" src/` devuelve **una**
      línea, contra las dos de hoy
- [ ] T030 Oráculo de **AC9**: `grep -rn "seis efectos" .` no devuelve nada fuera de `specs/` —los
...
- [ ] T031 La evidencia de **AC13** no es correr el typecheck del server —`pnpm verify` ya lo corre
...
- [ ] T034 Rama `feature/022-el-puente-con-el-motor-sale-del-shell`
- [ ] T035 Actualizar la fila del 022 en `specs/log.md` a `Implementado`
- [ ] T036 Anotar en `specs/revisiones.md` qué se aprendió. Candidato: que el ítem de deuda más viejo
