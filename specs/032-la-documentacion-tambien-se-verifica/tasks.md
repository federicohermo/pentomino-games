...
- [ ] T002 [P] Corregir el ancla de `docs/guides/mcp-domain.md` hacia `docs/guides/troubleshooting.md`:
      hoy apunta a `#el-mcp-server-no-arranca-err_module_not_found`, que ningún encabezado genera — **AC10**
- [ ] T003 [P] Agregar al árbol de `docs/architecture/directory-structure.md` los 5 archivos que no
      nombra: `src/audio/constants/{engine,scheduler,voice}.constants.ts`,
...
- [ ] T004 [P] `CLAUDE.md`: «Quedan **dos**» → «**tres**», nombrando `components/Board.tsx` y su motivo
      (el ancestro existe por construcción; el `if` alternativo sería rama inalcanzable), y «66» → «95» — **AC10**
- [ ] T005 [P] Los 23 fences sin lenguaje del carril A (13 en `docs/`, 8 en `.claude/`, 1 en la raíz,
      1 en `mcp-server/`). Sólo se agrega el lenguaje: no se toca el contenido — **AC1**
...
- [ ] T007 Bloque `**/*.md` en `eslint.config.js` con `language: 'markdown/gfm'` y
      `languageOptions: { frontmatter: 'yaml' }`, extendiendo `markdown/recommended`. El docblock del
...
- [ ] T009 Bloque `specs/[0-9]*/**/*.md` que apaga el preset y **reenciende por nombre** las reglas de
      ...
      del preset — **AC1**
- [ ] T010 Verificar que `pnpm lint` **sin globs** levanta los 155 `.md`: sin un bloque que matchee
...
- [ ] T013 [P] El mismo test **no** verifica la dirección inversa, y el comentario dice por qué: el doc
...
- [ ] T016 [P] Mismo archivo: biyección `log.md` ↔ carpetas, `href` que apunta a su propia carpeta,
      fecha ISO, estado en el conjunto cerrado que `log.md` declara arriba de su tabla — **AC8.3, AC8.4**
- [ ] T017 [P] Mismo archivo: **toda** línea que empieza como checkbox en un `tasks.md` parsea con el
...
- [ ] T018 [P] Mismo archivo: IDs `T###` únicos dentro de su spec, y **sin** exigir consecutividad ni
...
- [ ] T022 [P] `no-warning-comments` con `terms: ['v8 ignore', 'c8 ignore', 'istanbul ignore']` y
      `location: 'anywhere'`. Cero hallazgos hoy, así que entra gratis y vuelve gate el corolario del
      umbral 100 — **AC6**
...
- [ ] T030 [P] `specs/log.md`: la fila del 032
