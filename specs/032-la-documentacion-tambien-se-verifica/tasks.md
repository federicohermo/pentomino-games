...
- [x] T001 [P] Escapar las barras de `specs/027-lo-que-falla-en-silencio/research.md:112`: la fila
      declara 3 columnas y GFM le cuenta 5, así que **descarta** `Sale bien` al renderizar — **AC10**
- [x] T002 [P] Reescribir la frase de `vite.config.ts:155` para que **no deletree** el término que
...
- [x] T003 [P] Agregar al árbol de `docs/architecture/directory-structure.md` los **5** archivos que no
...
- [x] T004 [P] `CLAUDE.md`: «Quedan **dos**» → «**tres**», nombrando `components/Board.tsx` y su motivo
      ...
      **El número se escribe junto con la regla que lo produce**, porque sin ella no se reproduce: un
      ...
      conjunto — **AC10**
- [x] T005 Los **26** fences sin lenguaje del carril A (13 en `docs/`, 8 en `.claude/`, 1 en la raíz,
...
- [x] T006 `pnpm add -Dw @eslint/markdown` (8.0.3). Va a la raíz y no a `mcp-server`: el linter es uno
      solo para todo el repo
- [x] T007 Bloque `**/*.md` en `eslint.config.js` con `language: 'markdown/gfm'` y
      ...
      como H1 — **AC1**
- [x] T008 `no-missing-label-refs: 'off'` en ese bloque, con el desglose de las 341 en el comentario
...
- [x] T009 Bloque `specs/[0-9]*/**/*.md` que apaga el preset y **reenciende por nombre** las reglas de
      ...
      del preset — **AC1**
- [x] T010 Verificar que `pnpm lint` **sin globs** levanta **todos** los `.md` —hoy 162, y ESLint 9 sí
...
- [x] T011 [P] `src/__tests__/enlaces-resueltos.test.ts`: todo enlace relativo de los **162** `.md`
...
- [x] T012 [P] `src/__tests__/mapa-de-directorios.test.ts`: todo archivo de producción de `src/**` y de
...
- [x] T013 El mismo test **no** verifica la dirección inversa, y el comentario dice por qué: el doc
      tiene una sección «qué está muerto» que nombra `App.css`, `setupTests.ts` y `App.test.tsx`
      justamente porque ya no existen — **AC4**
- [x] T014 [P] `src/__tests__/claude-md-acotado.test.ts`: `CLAUDE.md` bajo 200 líneas, con la cita de
      `code.claude.com/docs/en/memory` en el mensaje de falla. **Este test falla hasta el paso 6** — **AC7**
- [x] T015 [P] `src/__tests__/specs-convencion.test.ts`, los cuatro archivos por carpeta y el nombre
      `NNN-kebab` — **AC8.1, AC8.2**
- [x] T016 Mismo archivo: biyección `log.md` ↔ carpetas, `href` que apunta a su propia carpeta,
      fecha ISO, estado en el conjunto cerrado que `log.md` declara arriba de su tabla — **AC8.3, AC8.4**
- [x] T017 Mismo archivo: **toda** línea que empieza como checkbox en un `tasks.md` parsea con el
...
- [x] T018 Mismo archivo: IDs `T###` únicos dentro de su spec, y **sin** exigir consecutividad ni
...
- [x] T019 `@typescript-eslint/no-non-null-assertion: 'error'` en el bloque `**/*.{ts,tsx}` — **AC5**
- [x] T020 Override que la apaga en `src/**/__tests__/**` y `mcp-server/**/__tests__/**`: ahí el `!`
...
- [x] T021 Override por archivo para las **tres** de producción —`src/main.tsx`,
...
- [x] T022 `no-warning-comments` con `terms: ['v8 ignore', 'c8 ignore', 'istanbul ignore']` y
...
- [x] T023 `pnpm verify` verde **salvo `claude-md-acotado.test.ts`**, que T014 deja rojo a propósito
      hasta el Paso 6. **Cuatro** nodos, ni uno más. El verde entero es T034, y no puede ser éste — **AC11**
- [x] T024 Medir `lint` y `suite` con caché caliente, antes y después, y escribir los cuatro números en
...
- [x] T025 Las nueve falsificaciones deliberadas del `plan.md`: romper una cosa por gate, ver el rojo,
      revertir. Un gate que nunca se vio fallar es un gate que no se sabe si anda
- [x] T026 [P] `specs/README.md`: la **Desviación 4** — el carril B existe porque la Desviación 2
      congela los specs, y las dos se leen juntas — **AC12**
- [x] T027 [P] `CLAUDE.md`: el régimen nuevo, corto, en la sección de reglas y en la de comandos
- [x] T028 [P] `docs/guides/conventions.md`: las dos reglas de TypeScript nuevas, con su motivo
- [x] T029 [P] **GitHub Issue** por la leniencia de `parseTasks`, con el motivo por el que hoy no se
...
- [x] T030 [P] `specs/log.md`: la fila del 032 **ya existe** (la commiteó `d0541e6`), así que la tarea
...
- [x] T031 `docs/guides/verificacion.md` nuevo: el detalle de `verify`, `suite`, los dos proyectos de
      Vitest, el umbral 100 y el linting con tipos, entero y sin recortar. Hoy no hay ningún doc que
      explique `verify` — **AC7**
- [x] T032 `## Comandos` de `CLAUDE.md` de 100 a ~25 líneas: se queda la afirmación y el número medido,
      se va el razonamiento. El criterio es el que el propio archivo escribió en su línea 3 — **AC7**
- [x] T033 `## Reglas que valen en todo el repo` de 76 a ~50: el razonamiento largo de cada bullet ya
      está en `docs/guides/conventions.md`, así que se enlaza en vez de repetirse — **AC7**
- [x] T034 El test de T014 en verde, y `pnpm verify` otra vez entero — **AC7, AC11**
...
- [ ] T039 **Los docblocks que deletreaban el término prohibido eran TRES, no uno.** El spec medía 1
      (`vite.config.ts:155`) y con la regla puesta salieron `mcp-server/src/tools/specStatus.ts:32` y
      `specWrite.ts:48` — y el segundo **lo creó el 033**, o sea que no existía cuando el research midió.
      Los tres son el mismo patrón y se arreglaron igual. Lo que hay que mirar de esto no es el arreglo
      sino que el número de un research envejece con cada spec que se mergea en el medio: el 033 entró
      entre la escritura del 032 y su implementación, y movió cuatro números de este spec.
- [ ] T040 **El T013 citaba una sección de `directory-structure.md` que no existe.** Decía «el doc tiene
      una sección "qué está muerto" que nombra `App.css`, `setupTests.ts` y `App.test.tsx`», y no hay
      ninguna sección con ese título: el contenido vive suelto en un párrafo de `## La dirección de
      dependencia` (`:200`), nombra `App.css`, `logo.svg`, `assets/react.svg` y `setupTests.ts`, y **no**
      nombra `App.test.tsx`. El test verifica el párrafo y no el encabezado, con el motivo al lado. El
      `CLAUDE.md` resume ese doc como «dónde crear cada cosa, qué está muerto», así que la frase venía de
      ahí y nadie la había ido a buscar.
- [ ] T041 **El gate de `log.md` verifica las DOS direcciones, y el del mapa de directorios sólo una.**
      La asimetría es deliberada y está escrita en los dos tests: en el mapa, la dirección inversa
      borraría el registro de los archivos eliminados a propósito; en `log.md` no hay equivalente, y una
      fila sin carpeta es un spec fantasma que `spec_status` reportaría como trabajo inexistente. El
      plan pedía falsificar «una fila de `log.md` sin carpeta» y el gate que la ataja no estaba: lo
      encontró la falsificación, que es para lo que existe.
- [ ] T042 **El costo de Markdown en `lint` son 2,5 s y el «antes» hubo que re-medirlo.** Comparar
      contra los 11,0 s que anotó el 030 no decía nada —otra máquina, otro momento—, así que se corrió
      `eslint .` con `--ignore-pattern "**/*.md"` en la misma sesión: 13,6 → 16,1 s. La conclusión del
      plan se sostiene con margen: `suite` sigue mandando el reloj con **33,8 s**, más del doble. Queda
      anotado que **un número de performance de un spec viejo no sirve como línea de base**, y que el
      research §9 escribe la regla que lo produce al lado de cada número.
