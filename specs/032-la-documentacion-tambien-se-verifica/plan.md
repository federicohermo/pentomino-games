...
2. `docs/guides/mcp-domain.md` — el ancla `#el-mcp-server-no-arranca-err_module_not_found`. Verificar
   contra los encabezados reales de `troubleshooting.md` y corregir el enlace (no el encabezado: el
   encabezado es el que está bien).
3. `docs/architecture/directory-structure.md` — agregar los 4 de `src/` (`audio/constants/`
   ...
   línea como los que ya tiene el árbol.
...
- Un bloque `files: ['**/*.md']` con `plugins: { markdown }`, `language: 'markdown/gfm'`,
  `languageOptions: { frontmatter: 'yaml' }` y `extends: ['markdown/recommended']`, más
...
- `no-warning-comments` con los tres términos y `location: 'anywhere'`.
