...
- [x] T001 `components/cell-name.ts`: el nombre accesible de una celda, armado desde lo que ya devuelven
...
- [x] T002 Docblock con por qué vive afuera del `.tsx`: adentro no se puede exportar y por lo tanto no se
      puede testear. Precedente exacto: `cell-text.ts`, que nació por esto mismo
- [x] T003 [P] `components/__tests__/cell-name.test.ts`: los cuatro casos
- [x] T004 `components/types/input.types.ts`: `EventoDeTecla` gana `targetEsCelda: boolean`, con el
...
- [x] T049 Y los dos conteos que quedan mal: `input.types.ts:15` dice «las cinco guardas» y `input.ts:73`
      «Las cuatro guardas, en orden». Con el campo nuevo son seis y cinco
- [x] T005 `input.ts::accionDeTecla`: con `targetEsCelda`, `' '` devuelve `null` — **AC5**
- [x] T006 **`Shift` y `Control` siguen devolviendo su acción con `targetEsCelda: true`** — **AC6**. No
...
- [x] T007 `input.ts::frenaElDefault`: la barra sigue frenando el default con la celda enfocada — el
      scroll hay que pararlo igual, lo maneje quien lo maneje
- [x] T050 **Las doce letras siguen pasando con `targetEsCelda: true`** — **AC15**. `targetEsCelda` apaga
      ...
      018 va a heredar
- [x] T008 `__tests__/input.test.ts`: la tabla de la asimetría de T005/T006 como oráculo explícito, con
      un comentario que diga que las tres filas juntas son el AC. Por separado no dicen nada
- [x] T009 `use-input.ts`: el predicado del DOM para la celda (`closest('[role="gridcell"]')`). Va en el
...
- [x] T010 `Board.tsx`: de 60 hijos planos a seis `role="row"` de diez `role="gridcell"`, sin `gap`,
      mismas medidas — **AC9**
- [x] T051 El `gridTemplateColumns` **se muda del contenedor a la fila** (`Board.tsx:197-198`): hoy las
...
- [x] T052 **Arreglar los cuatro selectores estructurales que el cambio rompe**, que hoy leen los hijos
...
- [x] T011 `role="grid"` en el contenedor de la grilla, con `aria-label` y `aria-rowcount`/`aria-colcount`
      leídos de `GRID_W`/`GRID_H` y no escritos a mano — **AC9**
- [x] T012 Comentario con por qué son **filas reales** y no `display: contents`: esa técnica ha sacado el
...
- [x] T013 Confirmar **AC11** con los tests que ya existen **y con el T052 aplicado**: la grilla mide
...
- [x] T014 Commit aparte. Es el único cambio del spec que puede mover el layout, y mezclado con
      `tabIndex` nuevos el diff deja de ser legible

## Paso 3 — El foco

- [x] T015 `App.tsx`: la celda enfocada **es** `hover`. Un solo estado, así que el fantasma, el cursor y
      `hoverEdita` funcionan con teclado sin una línea de dibujo nueva — **AC3**, D2
- [x] T062 Y decirlo al lado, porque es una arista con el **027**: esto le da a `hover` un **segundo
...
- [x] T063 Si el T015 toca el comentario de `src/App.tsx:253-257` —el que explica por qué los dos objetos
...
- [x] T016 Roving tabindex: `tabIndex={0}` en la celda del cursor y `-1` en las otras 59. Con el foco
      afuera, el `0` va a una celda de arranque para que el tablero siga siendo alcanzable — **AC1**
- [x] T017 Flechas: mueven una celda sin salirse de la grilla, y llaman a `preventDefault`. Sin eso la
      flecha scrollea la página **y** el `overflow-x-auto` del tablero — mismo trato que la rueda, mismo
      motivo — **AC2**
- [x] T018 [P] `Home` / `End`: primera y última celda de **su fila**, sin salirse de ella — **AC14**
- [x] T019 `onBlur` del contenedor: si el foco se fue del tablero, `hover` vuelve a `null` — lo mismo que
      hace hoy `onMouseLeave`
- [x] T053 **Y la regla de desempate al revés**: mientras el foco del DOM esté ADENTRO del tablero, el
...
- [x] T020 [P] `layout.constants.ts`: los dos anchos del anillo de foco. Los módulos no declaran
      constantes
- [x] T021 El anillo va en la **caja de afuera**, y el comentario dice las dos cosas: que es el único
...
- [x] T022 Dos tonos, claro adentro y oscuro afuera: abajo puede haber `#FFFF00` (`V`) o `#0000FF` (`W`)
      y un solo color no cubre los dos extremos
- [x] T055 Y por eso son **dos propiedades**: un `outline` de CSS tiene un solo color. `outline` para el
...
- [x] T024 `Enter` y `Espacio` llaman a **`accionDeClick`**, la misma pura que el click, con los mismos
      argumentos. **Sin una segunda copia de la regla** — **AC4**
- [x] T025 `Alt`+`Enter` y `Alt`+`Espacio` pasan `altKey: true` a esa misma pura: mutear y colocar
...
- [x] T026 Comentario: si mañana `Alt` cambia de significado, el teclado lo hereda sin que nadie se
      acuerde de tocarlo. Es lo que hace que la tabla del spec no sea una promesa
- [x] T027 `App.tsx`: región `aria-live="polite"` con el resultado de las tres ediciones — colocar,
...
- [x] T028 Y **nada más** en esa región: ni el recorrido, ni la cabeza lectora, ni el espectro. Anunciar
      a 10 Hz es hostil, y ya está anotado como seguimiento del 025
- [x] T029 El texto del anuncio sale de `cell-name.ts` y del nombre de la pieza, no de una cadena escrita
      a mano en el shell
...
- [x] T030 `src/__tests__/App.browser.test.tsx`: desde la paleta, **un** `Tab` llega al tablero y **otro**
      lo pasa de largo — **AC1**, **AC12**
- [x] T031 Mismo archivo: las flechas mueven el foco y el `body` no scrollea — **AC2**
- [x] T057 Mismo archivo: `Home` y `End` van a la primera y a la última celda de su fila — **AC14**
- [x] T032 Mismo archivo: `Enter` coloca, `Enter` sobre la propia pieza quita, `Alt`+`Enter` mutea —
      **AC4**
- [x] T033 Mismo archivo: con una celda enfocada, la barra **no** alterna el transporte; con un
      `<button>` enfocado tampoco; con el foco en el `body`, sí — **AC5**
- [x] T034 Mismo archivo: con una celda enfocada, `Shift` **sí** rota y `Ctrl` **sí** refleja — **AC6**.
      Va separado del T033 a propósito: uno verifica que se apagó, el otro que no se apagó de más
- [x] T058 Mismo archivo: con el foco en una celda, sacar el mouse de la grilla **no** apaga el fantasma
      — **AC16**
- [x] T059 [P] `components/__tests__/Board.browser.test.tsx`: la grilla es `role="grid"` con seis
...
- [ ] T035 [M] Recorrer el tablero entero con teclado y un lector de pantalla: colocar, quitar y mutear
      sin tocar el mouse
- [x] T036 `specs/deuda.md`: se borra el ítem «El tablero no se puede tocar con el teclado»
- [x] T037 **«Tampoco hay deshacer» NO se va con él**: se separa en su propio ítem, y con el argumento de
      que este spec lo hace **más** necesario — ahora la operación destructiva también se alcanza sin
      querer, con una tecla
- [x] T038 [P] `.claude/rules/ui.md`, **debajo de lo que ya escribió el 025**: el 025 va primero y deja
...
- [x] T060 Y en el mismo archivo (sin `[P]`: es el mismo `ui.md` del T038), la guarda del handler global (`.claude/rules/ui.md:143-145`) gana
...
- [x] T039 [P] `DESIGN.md`, **debajo de lo que ya escribió el 025**: ese párrafo lo toca el 025 primero
...
- [x] T061 Y la frase (sin `[P]`: es el mismo `DESIGN.md` del T039) que hace que las dos convivan, que hoy no está dicha en ningún lado: **el
