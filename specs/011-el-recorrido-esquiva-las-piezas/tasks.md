...
- [ ] T005 **Crear rama** `feature/011-pisar-una-pieza-cuesta`
...
- [ ] T015 `routeBetween(a, b, placed)` en `domain/board.ts`: camino de costo mínimo, peso 1 en celda vacía y
      `P` en ocupada, con la arista de la costura. Devuelve **camino, pasos y cruces en una sola
      llamada** (D3)
- [ ] T016 **El costo ordena, los pasos miden el tiempo.** Un cruce cuesta `P` pero dura **un** intervalo. Si
      el costo se filtra a los offsets, el ciclo se estira donde no debe
- [ ] T017 **El peso lo pagan las celdas INTERMEDIAS, no las dos puntas.** Las puertas están sobre una pieza
...
- [ ] T030 La derivación celda→nota va en una **pura del dominio**, no adentro de `buildSequence` (la razón
      es la misma por la que `cellsByPlayOrder` salió de adentro de `gates` en el 010)
- [ ] T031 Cuidado con las dos trampas de la cadena: `degreeByCellIndex` sobre la forma **canónica** (sobre
      ...
      retrógrado aplicado
- [ ] T032 **Ojo con el nombre:** el arpegio con retrógrado era `PlacedPiece.notes` cuando se escribió este
...
- [ ] T033 [P] `Click` puede llevar altura — `domain/types/sequence.types.ts`
- [ ] T034 **NO agregar** la garantía de que un cruce no coincida con una nota: ya vale por construcción y el
      test del 009 la cubre
- [ ] T035 [P] Test AC3 sobre la **`X`**, que es el caso estructural
- [ ] T036 `check_invariants` antes y después

## Paso 3 — El motor toca esa nota
- [ ] T037 El cruce viaja al motor con altura MIDI, sin `Cell` — `audio/types/scheduler.types.ts`
- [ ] T038 **Tercera clase de evento** (AC13): `HIT` pasa de `{ note, click }` a tres claves en
...
- [ ] T039 [P] `App.tsx` sigue **proyectando y no traduciendo**
- [ ] T040 [P] `GRACE_INTERVALS` y `GRACE_VELOCITY` en `voice.constants.ts`. `GRACE_VELOCITY` va al lado de
...
- [ ] T045 [P] `Playhead.tsx`: tres escalones de grosor de borde, **sin agregar color**

## Paso 5 — Las tools
- [ ] T046 `simulate_board` reporta los cruces y sus notas. Es además la forma barata de comparar valores de
      `P` sin escuchar cada uno
- [ ] T047 **La descripción de la tool afirma lo contrario de lo que va a pasar** y es contrato, no prosa: un
...
## Documentación
- [ ] T051 [P] `.claude/rules/audio.md`: la línea «un click puede caer sobre una pieza — los 21 del ciclo, en el
...
- [ ] T055 [P] `DESIGN.md`: el tercer escalón de la cabeza. La sección «La cabeza lectora: el estado va al borde»
...
## Verificación
- [ ] T057 [P] `pnpm verify` en verde (AC9). Es el nodo que atrapa el borde de paquete: typechequea `mcp-server/`
      contra `src/`, y ahí viven los dos asserts de `tools.test.ts` que importan `cellDistance` y
      `pathBetween`
- [ ] T058 [P] `check_invariants` en **proceso fresco**, antes y después
...
- [ ] T062 [P] **Ningún símbolo queda huérfano**: `ROUTE`, `RouteKind` y lo que el borrado de `bestRoute` arrastre.
      `find_symbol` responde `usedBy` en una consulta

## PR
- [ ] T063 **Declarar el cambio de audio arriba de todo**: cambia la matriz de costos y con ella el orden de
      visita en el 30-48 % de los tableros. No es una mejora visual
- [ ] T064 Aclarar que `cellDistance(a, b)` de dos argumentos **ya no existe**: es el cambio conceptual
- [ ] T065 Aclarar que **no hay regla de esquivar con excepción, hay un peso** — y que la primera versión del
...
- [ ] T066 La tabla de P, que es lo que hace revisable la elección
- [ ] T067 Declarar el **borde de paquete**: `mcp-server/` importa `cellDistance` y `pathBetween`, así que la
