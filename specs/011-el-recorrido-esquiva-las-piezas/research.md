...
| **Quien CONSTRUYE los `Hit`** — `collectWindow` los arma: `:144` la nota, `:150` el click | `audio/scheduler.ts` | La tercera clase se emite acá, no en `engine.ts`, que solo despacha (`:302`). Faltaba en esta tabla y en las tareas (AC14) |
| El docblock de `Sequence` y el comentario del campo `clicks` argumentan que al motor le alcanza con CONTAR | `audio/types/scheduler.types.ts` | Con el cruce con altura esa mitad deja de valer; la otra —no ver `Cell`— sigue valiendo (AC14) |
| Los docblocks que explican `SEAM` y el invariante del largo en términos de `bestRoute` y `ROUTE` | `domain/constants/board.constants.ts:26-28`, `domain/sequence.ts:188` y `:68`, y los comentarios de `domain/__tests__/sequence.test.ts:106` y `:182` | Mueren con el borrado, y quedan nombrando símbolos que ya no existen (AC14) |
| **Los 13 tests de `cellDistance` y `pathBetween`**, cinco de ellos sobre propiedades que el modelo nuevo cambia | `domain/__tests__/board.test.ts:186-308` | Son los consumidores directos de las dos funciones que este spec reescribe (AC12) |
| Los usos indirectos en los tests de la secuencia | `domain/__tests__/sequence.test.ts:66`, `:150-151`, `:439-440`, con el `import` de `:3` | Ídem, vía `buildSequence` |
