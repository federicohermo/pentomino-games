...
- [ ] Verificar que el **009 esté mergeado**: sin recorrido no hay nada que seguir. Ya lo está
...
- [ ] `cellsByPlayOrder(p)` en `domain/sequence.ts`, con el retrógrado ya aplicado (D8)
- [ ] Test AC11: sobre las 96 combinaciones, `cellsByPlayOrder(p)[j]` es la celda de `p.notes[j]`
- [ ] **Commit propio.** Solo la pura y su test: todavía no cambia nada de lo que suena
- [ ] `gates` pasa a leer de `cellsByPlayOrder` — `{ entrada: orden[0], salida: orden.at(-1) }` (D9)
- [ ] Test AC12: con `mirror`, `entrada` es la celda de la primera nota. Caso testigo `L`/0/reflejada:
      entrada `[0,0]`, salida `[1,3]` (hoy da al revés)
- [ ] `check_invariants` antes y después (lo obliga `.claude/rules/domain.md` al tocar esta capa)
- [ ] `simulate_board` sobre el caso testigo: ver el circuito darse vuelta
- [ ] **Commit propio y atribuido al 009** (AC13). Es el único que cambia lo que suena
- [ ] Verificar los dos consumidores de `gates`: `buildSequence` (matriz de costos) y `simulate_board`
      (lo reporta por pieza). Ninguno cambia, pero sus salidas sí

## El motor
- [ ] La aritmética del offset como **pura testeable** en `audio/playhead.ts`, separada de la lectura
      del singleton (precedente: `spectrum.ts`)
- [ ] Test AC2 en `src/audio/__tests__/playhead.test.ts` — offset dentro del primer ciclo, en el borde,
      y varios ciclos adelante
- [ ] Test AC2, degradados: **ciclo 0** (`x mod 0` es `NaN`), **`t` anterior al `origin`** (el módulo de
...
- [ ] Cadena de latencia: `outputLatency` → `baseLatency` → 0 (D4). Sin `any` ni `@ts-ignore`:
...
- [ ] **AC9** — `components/route-source.ts`: el par activa/pendiente del dominio, fuera de React, con
      el swap atado a `cycleGeneration()`. Sin esto la cabeza dibuja el circuito encolado mientras
      suena el viejo
- [ ] `App.tsx`: el efecto de reconciliación encola en las **dos** colas —`setSequence` y `encolar`—,
      desde el mismo lugar y con la misma `buildSequence`
- [ ] Verificar el primer arranque: `cycleGeneration()` tiene que subir también en el swap inicial, o
...
- [ ] `Board` importa `Playhead` y lo monta en su `relative overflow-x-auto` (`Board.tsx:127`). **No**
      se devuelve la ranura de `children` que el review del 007 retiró
- [ ] Traducir offset → celda desde `rutaActiva()`, sin llamar a `occupantAt`: `Click.cell` para el
...
- [ ] La pieza pendiente se ve atenuada hasta el swap de ciclo (AC5), **por React**: el loop llama a
...
## Documentación
- [ ] `docs/architecture/modelo-musical.md`: la tabla dice "Reflexión → el orden de las notas
      (retrógrado)". Con D9 la reflexión también decide **por dónde entra y sale el recorrido** — hoy
      la doc describe medio efecto
- [ ] `specs/log.md`, nota de revisión: el bug de las puertas con reflexión, cómo se encontró (medir de
...
- [ ] `docs/architecture/modelo-musical.md:73`: retirar la "Limitación conocida" — este spec la cierra
- [ ] `.claude/rules/domain.md:49`: la misma frase, escrita una segunda vez ("hoy se oye pero no se lee
...
- [ ] `.claude/rules/ui.md:15-16`: `Spectrum` deja de ser la única excepción; la regla es la misma
- [ ] `.claude/rules/audio.md`: el `paths:` del frontmatter lista `src/components/Spectrum.tsx` uno por
      uno — agregar `Playhead.tsx` o la regla no se carga al tocarlo
- [ ] `docs/architecture/directory-structure.md`: `audio/playhead.ts` y `components/Playhead.tsx`
- [ ] `App.tsx:117`: el comentario dice que el 010 va a leer las celdas de `placed`. Con AC9 eso es
      exactamente lo que no alcanza — corregirlo o queda como pista falsa
- [ ] `specs/log.md`: estado del 010 (y el del 009, ver Backlog)
...
- [ ] AC7 — en pausa no se dibuja cabeza y el loop no escribe ningún estilo
...
- [ ] **AC10** (no-regresión) — el fantasma sigue mostrando nota y grado por celda; "Quitar" y "Reset"
...
- [ ] **AC11 / AC12 / AC13** — los tests del paso 0 en verde y el arreglo de D9 en su propio commit
...
- [ ] Degradados a mano: tablero vacío con play apretado, y una sola pieza (sin clicks, sin saltos)
...
- [ ] **Declarar el cambio de audio arriba de todo**: el paso 0b arregla las puertas con reflexión y
