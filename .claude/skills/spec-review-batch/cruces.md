...
un rebase. Acá el spec todavía es texto — el hallazgo se escribe en un `tasks.md` y listo.
...
- **Se detecta** buscando los `X → Y` de cada `tasks.md` y cruzando los `Y` contra los `X` del resto.
...
Es el único caso en que un spec escribe fuera de su propia carpeta, así que es también el único que dos
agentes pueden pisar a la vez — por eso lo aplica el padre, nunca el agente.
...
- **Se edita** en los dos lados: la tarea del spec viejo, y el spec nuevo declarando que la cierra y por
  qué. Cerrarla sólo de un lado deja `spec_status` reportando trabajo que ya no existe.
